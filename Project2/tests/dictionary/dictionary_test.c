...
void test_find(void){
    ...
    for(int i=0;i<10000;i++){
        ...
        // Start with 1 to not double check whether it exists once __so payload would be 1__, or more 
        int count=1;
        for(int j=0;j<10000;j++){
            if(strcmp(strings[i],strings[j])==0 && j!=i){
                count++;
            }
        }
        TEST_ASSERT(entry->payload->size==count);
    }
    for(int i=0;i<10000;i++){
        free(strings[i]);
    }
